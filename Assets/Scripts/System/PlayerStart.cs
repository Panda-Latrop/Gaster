...
public class PlayerStart : MonoBehaviour
{
    ...
    protected void OnDrawGizmos()
    {
        ...
        if (!Application.isPlaying)
        {
            ...
            Gizmos.DrawSphere(transform.position + Vector3.back, 0.75f);
            Gizmos.DrawLine(transform.position, transform.position + (transform.rotation * Vector3.right) * 2);
            for (int i = 0; i < points.Count; i++)
            {
                Gizmos.DrawSphere(points[i].position + Vector3.back, 0.75f);
                Gizmos.DrawLine(points[i].position, points[i].position + (points[i].rotation * Vector3.right) * 2);
            }
        }
    }
}
